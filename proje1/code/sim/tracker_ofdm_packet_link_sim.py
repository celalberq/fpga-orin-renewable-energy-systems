#!/usr/bin/env python3
"""OFDM packet-link simulation using the Orin tracker telemetry frame."""
...
from qpsk_packet_link_sim import bits_to_bytes, bytes_to_bits
from tracker_packet_profile import make_tracker_frame, tracker_frame_ok


PROJECT_ROOT = Path(__file__).resolve().parents[2]
OUT_PATH = PROJECT_ROOT / "data" / "tracker_ofdm_packet_link_sim.csv"
GRID_PATH = PROJECT_ROOT / "data" / "tracker_ofdm_resource_grid_example.csv"
...
def add_awgn(samples: list[complex], ebn0_db: float, rng: random.Random) -> list[complex]:
    ...
    return [
        complex(sample.real + rng.gauss(0.0, noise_sigma), sample.imag + rng.gauss(0.0, noise_sigma))
...
def write_resource_grid_example() -> None:
    grid = build_ofdm_symbols(make_tracker_frame(23))[0]
