#!/usr/bin/env python3
"""Bridge live Orin tracker UDP packets into the proje1 comm frame profile.

This tool keeps the three projects tied together:
  proje3 sends rich tracker/INA226 JSON.
  proje1 converts it into a compact protected telemetry frame profile.
  proje2 can still receive the original JSON through UDP forwarding.
"""
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Convert live Orin tracker UDP JSON into proje1 comm frame profiles.")
    ...
    parser.add_argument("--log", type=Path, default=Path("proje1/data/tracker_udp_comm_bridge_log.csv"))
