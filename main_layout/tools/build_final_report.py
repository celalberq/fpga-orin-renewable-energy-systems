...
OUT_DIR = ROOT / "main_layout" / "demo_evidence" / "final_report"
TMP_DIR = OUT_DIR / "_build"
OUT_FILE = OUT_DIR / "fpga_orin_solar_bess_final_report_2026_08_11.docx"
...
def crop_charge_image():
    source = ROOT / "main_layout" / "demo_evidence" / "image" / "bess_real_charge_dashboard_2026-08-11.png"
...
def build_document():
    ...
    set_run(header.add_run("INTEGRATED RENEWABLE TELEMETRY PLATFORM"), size=8.5, color=MUTED, bold=True)
    ...
    set_run(p.add_run("FPGA and Jetson Orin Solar Tracker + Bidirectional BESS Telemetry"), size=23, color=INK, bold=True)
    p = doc.add_paragraph()
    ...
    for label, value in (
        ...
        ("Integrated layers", "Proje3 renewable control | Proje1 digital communication | Proje2 network monitoring"),
    ...
    add_body(doc, "This project demonstrates a complete low-voltage renewable edge prototype. Four light sensors are sampled through an MCP3208 and Nexys Video FPGA; a Jetson Orin estimates light direction and controls a physical two-axis solar-panel mount; and an INA226 measures a protected 1S Li-ion battery during supervised charging and fan-load discharge. The same live packet is profiled by a CRC/QPSK/OFDM communication layer and monitored by a UDP dashboard with packet validation, gap detection, protection state, and CSV evidence.")
    add_metric_table(doc, (("Tracking gain", "+24.141%"), ("Final frames", "368 / 368"), ("Sequence gaps", "0"), ("BESS modes", "3")))
    add_body(doc, "The result is best presented as one system rather than three unrelated exercises: renewable sensing and control from Proje3, deterministic communication framing from Proje1, and network observability from Proje2. The final run preserved those functions while the measured battery state changed continuously from idle to charging, through a safe idle transition, to discharging, and back to idle.")
    ...
    add_heading(doc, "Project-Layer Ownership", 2)
    table = doc.add_table(rows=1, cols=3)
    table.style = "Table Grid"
    set_table_geometry(table, [1800, 3780, 3780])
    for i, text in enumerate(("Layer", "Primary responsibility", "Validated evidence")):
        set_cell_shading(table.cell(0, i), LIGHT)
        set_run(table.cell(0, i).paragraphs[0].add_run(text), size=9.5, color=DARK_BLUE, bold=True)
    set_repeat_table_header(table.rows[0])
    for layer, responsibility, evidence in (
        ("Proje3", "LDR and panel sensing; Orin tracking; PCA9685 servo actuation; INA226 BESS measurement; protection semantics", "Two-axis tracking, shading/recovery, charge/discharge, unified live run"),
        ("Proje1", "CRC frame construction; QPSK sizing; OFDM resource, pilot, padding, and cyclic-prefix accounting", "Nexys Video board tests and live 161-byte mixed-frame bridge"),
        ("Proje2", "UDP transport; packet validation; gap monitoring; dashboard state; CSV logging", "Healthy live link, zero-gap runs, reliability warning/recovery"),
    ):
        cells = table.add_row().cells
        for i, text in enumerate((layer, responsibility, evidence)):
    ...
    image = ROOT / "main_layout" / "reports" / "assets" / "demo_final_full.png"
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    ...
    for label, value in (
        ("Final bidirectional video", "main_layout/demo_evidence/video/unified_tracker_bess_bidirectional_final_demo_2026-08-11.mp4"),
        ("Final bridge CSV", "proje1/data/unified_tracker_bess_bidirectional_demo_bridge.csv"),
        ("Final dashboard CSV", "proje2/data/unified_tracker_bess_bidirectional_demo_dashboard.csv"),
        ("Final board-test record", "proje3/docs/board_test_25_unified_tracker_bess_bidirectional_demo.txt"),
    ...
    add_body(doc, "The completed prototype demonstrates physical sensing, real two-axis control, panel-power estimation, measured battery charging and discharge, FPGA communication validation, reliable UDP transport, and dashboard observability in one coherent system. It provides portfolio evidence across FPGA design, embedded Linux, instrumentation, renewable energy, digital communications, networking, and data-driven verification.")

    doc.core_properties.title = "FPGA and Jetson Orin Solar Tracker + Bidirectional BESS Telemetry"
    doc.core_properties.subject = "Final integrated technical report"
