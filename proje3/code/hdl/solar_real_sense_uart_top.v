...
module solar_real_sense_uart_top #(
    parameter integer CLK_HZ = 100000000,
    parameter integer BAUD = 115200,
    parameter integer SEND_INTERVAL_CLKS = 100000000,
    parameter integer ADC_SAMPLE_INTERVAL_CLKS = 1000000
) (
    input wire clk,
    input wire [7:0] sw,
    ...
    reg adc_start = 1'b0;
    reg adc_channel_select = 1'b0;
    reg [11:0] adc_voltage_raw = 12'd0;
    reg [11:0] adc_current_raw = 12'd0;

    wire adc_busy;
    wire adc_done;
    wire [11:0] adc_sample_data;
    wire [2:0] adc_channel = adc_channel_select ? 3'd1 : 3'd0;
    ...
    wire [31:0] voltage_scaled_mv = ({20'd0, adc_voltage_raw} * 32'd18300) / 32'd4095;
    wire [31:0] current_scaled_ma = ({20'd0, adc_current_raw} * 32'd3300) / 32'd4095;
    wire [15:0] voltage_mv_wire = (voltage_scaled_mv > 32'd65535) ? 16'hFFFF : voltage_scaled_mv[15:0];
    wire [15:0] current_ma_wire = (current_scaled_ma > 32'd65535) ? 16'hFFFF : current_scaled_ma[15:0];
    wire [31:0] power_calc_wire = voltage_mv_wire * current_ma_wire;
    wire [31:0] power_scaled_mw = power_calc_wire / 32'd1000;
    wire [15:0] power_mw_wire = (power_scaled_mw > 32'd65535) ? 16'hFFFF : power_scaled_mw[15:0];

    wire fault_wire = sw[7] || (duty_raw >= 8'hF0) || (voltage_mv_wire > 16'd18000) || (current_ma_wire > 16'd3000);
    ...
    always @(posedge clk) begin
        ...
        if (adc_done) begin
            if (adc_channel_select) begin
                adc_current_raw <= adc_sample_data;
            end else begin
                adc_voltage_raw <= adc_sample_data;
            end
