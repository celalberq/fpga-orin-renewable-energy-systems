#!/usr/bin/env python3
"""Bridge solar-tracker-bess solar UART packets to a UDP JSON telemetry stream."""
...
def require_pyserial() -> None:
    if serial is None or serial_list_ports is None:
        raise SystemExit(
            ...
            "python -m pip install -r system-integration/tools/requirements.txt"
        )


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate solar-tracker-bess solar telemetry and forward it as UDP JSON for network-telemetry-dashboard."
    )
    ...
    parser.add_argument("--log", type=Path, default=Path("network-telemetry-dashboard/data/solar_udp_gateway_log.csv"))
