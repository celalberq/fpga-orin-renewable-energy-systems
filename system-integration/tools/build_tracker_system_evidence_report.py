...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--dashboard-log", type=Path, default=Path("network-telemetry-dashboard/data/solar_live_dashboard_ldr_servo_log.csv"))
    parser.add_argument("--bridge-log", type=Path, default=Path("fpga-digital-communications/data/ldr_tracker_udp_comm_bridge_servo_log.csv"))
    parser.add_argument("--ab-metrics", type=Path, default=Path("solar-tracker-bess/data/solar_tracking_ab_metrics.csv"))
    parser.add_argument(
        "--fault-dashboard-log",
        type=Path,
        default=Path("network-telemetry-dashboard/data/solar_live_dashboard_ldr_fault_final.csv"),
    )
    parser.add_argument("--out-dir", type=Path, default=Path("system-integration/reports"))
...
def write_summary(path: Path, metrics: list[tuple[str, str]], board_test_10_passed: bool) -> None:
    ...
    lines = [
        ...
        "-------",
        "",
        "Summarize the current integrated renewable-energy tracker evidence across solar-tracker-bess, network-telemetry-dashboard, and fpga-digital-communications.",
        "",
        "Integrated Path",
        "---------------",
        "",
        "Nexys/MCP3208 LDR sensing -> Orin tracker/PCA9685 actuation -> fpga-digital-communications compact CRC frame/QPSK/OFDM profile -> network-telemetry-dashboard UDP dashboard.",
        "",
        "Key Results",
        "-----------",
        "",
...
def main() -> int:
    ...
    board_doc = Path("fpga-digital-communications/docs/board_test_10_tracker_frame_qpsk_ofdm_batch.txt")
